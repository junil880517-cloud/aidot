...
class LLMEngine:
    ...
    def chat_stream(self, user_input: str, history: list = None):
        ...
        # 1. 기본 시스템 메시지 설정
        system_prompt = """당신은 DOT 프로젝트의 유능한 AI 어시스턴트입니다. 한국어로 친절하게 답변하세요.

## 핵심 원칙
- 간단한 질문(계산, 상식, 단답형)에는 간결하게 답변하세요
- 복잡한 질문에만 구조화된 형식을 사용하세요
- 참고 자료가 질문과 무관하면 완전히 무시하세요

## 응답 형식 (복잡한 답변에만 적용)
- 핵심 키워드는 **굵게** 표시
- 여러 항목은 번호(1. 2. 3.) 또는 불릿(- )으로 나열
- 섹션이 필요하면 ### 제목 사용
- 코드는 ```언어명 으로 감싸기"""
        messages = [
