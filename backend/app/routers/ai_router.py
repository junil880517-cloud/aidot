...
async def chat_stream_endpoint(req: ChatStreamRequest):
    ...
    # threshold=0.9로 관련성 낮은 문서 필터링 (L2 거리 기반, 낮을수록 유사)
    search_results = rag.search(user_msg, k=3, threshold=0.9)

    if search_results:
        context_text = "\n".join([res['content'] for res in search_results])
        final_input = f"""[참고 자료]\n{context_text}\n\n[질문]\n{user_msg}\n\n위 자료가 질문과 관련 있으면 참고하여 답변하고, 관련 없으면 무시하고 직접 답변하세요."""
    else:
        ...
        while True:
            ...
            if item:
                ...
                if value.startswith("DOCS:"):
                    ...
                    # 줄바꿈 문자를 이스케이프하여 SSE 파싱 문제 방지
                    escaped_token = value[5:].replace('\n', '\\n').replace('\r', '\\r')
                    yield f"TEXT_DATA:{escaped_token}\n\n"
