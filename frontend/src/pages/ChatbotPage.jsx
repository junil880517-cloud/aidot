    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            while (true) {
                ...
                // SSE 이벤트 구분자 \n\n로 분리하되, 실제 데이터 내의 줄바꿈은 보존
                const events = chunk.split('\n\n');

                for (const event of events) {
                    if (!event.trim()) continue;

                    if (event.startsWith('TEXT_DATA:')) {
                        // 백엔드에서 이스케이프된 줄바꿈 문자를 복원
                        const token = event.slice(10).replace(/\\n/g, '\n').replace(/\\r/g, '\r');
                        fullResponse += token;
                        setStreamingMessage(fullResponse);
                    } else if (event.startsWith('DOCS_DATA:')) {
                        try {
                            docs = JSON.parse(event.slice(10));
                            setReferenceDocs(docs);
                        } catch (e) {
                            console.error('문서 파싱 실패:', e);
                        }
                    } else if (event.startsWith('ERROR_DATA:')) {
                        console.error('에러:', event.slice(11));
                    } else if (event === 'STOPPED_DATA:') {
                        console.log('생성 중단됨');
                        break;
                    }
                }
